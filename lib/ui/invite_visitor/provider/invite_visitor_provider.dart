...
import 'package:pif_flutter/ui/invite_visitor/popup/previous_visitor_popup.dart';
...
class InviteVisitorNotifier extends StateNotifier<InviteVisitorState> {
  ...
  void sendInvitation(BuildContext context) {
    ...
    if (state.startDate.isAfter(state.endDate)) {
      errorMessage(context: context, errorMessage: S.current.dateTimeCompareMsg);
      return;
    }
  ...
  void openStartDatePickerDialog() {
    if (state.isOpenStartTimePicker || state.isOpenEndTimePicker || state.isOpenEndDatePicker) {
      return;
    }
  ...
  void openStartTimePickerDialog() {
    if (state.isOpenStartDatePicker || state.isOpenEndTimePicker || state.isOpenEndDatePicker) {
      return;
    }
