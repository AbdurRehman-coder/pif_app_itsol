...
class AwesomeCalendar extends StatefulWidget {
  ...
  final void Function(DateTime pageStartDate, DateTime pageEndDate)? onPageSelected;
...
class AwesomeCalendarState extends State<AwesomeCalendar> {
  ...
  void initState() {
    ...
    pagesCount = CalendarHelper.calculateMonthsDifference(startDate, endDate) + 1;
    final int maxWeeksNumber = CalendarHelper.calculateMaxWeeksNumberMonthly(startDate, endDate);
  ...
  Widget build(BuildContext context) {
    ...
    //   initialPage: selectedSingleDate != null ? _getPageForDate(selectedSingleDate!) : 0,
    // );
    return SizedBox(
      height: widgetHeight,
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          final DateRange pageDateRange = _calculatePageDateRange(index);
  ...
  int _getPageForDate(DateTime date) {
    return (date.year * 12 + date.month) - (startDate.year * 12 + startDate.month);
  }
  ...
  DateRange _calculatePageDateRange(int pagePosition) {
    ...
    if (pagePosition == 0) {
      ...
      if (pagesCount <= 1) {
        ...
        final DateTime lastDayOfMonth = CalendarHelper.getLastDayOfMonth(startDate);
      ...
      final DateTime firstDateOfCurrentMonth = CalendarHelper.addMonths(startDate, pagePosition)!;
